...
if (!empty($assignedSection)) {
    $stmt = $conn->prepare("SELECT * FROM students_data WHERE stud_section = ? AND is_working_student = 'no' AND verification_status='accept'");
        ...
        $(document).ready(function() {
            $('.search-btn').on('click', function() {
                ...
                $('#table-' + tableId + ' tbody tr').each(function() {
                    var fullName = $(this).find('td:eq(2)').text().toLowerCase(); // Search by Full Name (3rd column)
