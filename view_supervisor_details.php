...
try {
    $stmt = $conn->prepare("
        ...
        WHERE 
            student_id = ? AND document_type = 'supervisor_details'
        ORDER BY
            upload_date DESC
        LIMIT 1
    ");
    $stmt->execute([$student_id]);
    $supervisorDetails = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
        ...
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            ...
            <div class="content-holder">
                <div class="file-overview-holder">
                    <fieldset>
                        <legend>Supervisor Details Document</legend>
                        <div class="overview-cont">
                            <?php if ($supervisorDetails): ?>
                                <div class="moa-preview" id="supervisorDetailsPreview">
                                    <?php
                                    $file_ext = strtolower(pathinfo($supervisorDetails['uploaded_path'], PATHINFO_EXTENSION));
                                    if ($file_ext === 'pdf'): ?>
                                        <iframe src="<?php echo $supervisorDetails['uploaded_path'] . '?t=' . time(); ?>" frameborder="0"></iframe>
                                    <?php elseif (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                        <img src="<?php echo $supervisorDetails['uploaded_path']; ?>" alt="MOA Image">
                                    <?php else: ?>
                ...
                <div class="info-function-cont">
                        ...
                        <div class="moa-overview">
                            <?php if ($supervisorDetails): ?>
                                <div class="data-row">
                                    ...
                                    <div class="student-data">: <?php echo htmlspecialchars($supervisorDetails['document_name']); ?></div>
                                </div>
                                <div class="data-row">
                                    <div class="label"><p><b>Status</b></p></div>
                                    <div class="student-data">: <?php echo htmlspecialchars(ucfirst($supervisorDetails['status'])); ?></div>
                                </div>
                            <?php else: ?>
                                <div class="data-row">
                    ...
                    <div class="button-function">
                        <button class="btn-style" onclick="viewMoa()" <?php echo $supervisorDetails ? '' : 'disabled'; ?>>
                            <i class="fa-regular fa-eye"></i> View
                        </button>
                        <button class="btn-style" onclick="updateStatus(<?php echo $supervisorDetails['document_id'] ?? 'null'; ?>, 'accepted')" <?php echo $supervisorDetails && $supervisorDetails['status'] !== 'accepted' ? '' : 'disabled'; ?>>
                            <i class="fa-solid fa-circle-check"></i> Approve
                        </button>
                        <button class="btn-style" onclick="toggleReject()" <?php echo $supervisorDetails && $supervisorDetails['status'] !== 'denied' ? '' : 'disabled'; ?>>
...
function viewMoa() {
    <?php if ($supervisorDetails): ?>
        ...
        const supervisorDetailsPreview = document.getElementById('supervisorDetailsPreview');
        const overlay = document.getElementById('overlay');

        if (fileExt === 'pdf') {
            supervisorDetailsPreview.classList.toggle('enlarged');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    remarksBtn.addEventListener('click', function() {
        const documentId = <?php echo $supervisorDetails['document_id'] ?? 'null'; ?>;
    ...
    overlay.addEventListener('click', function() {
        const supervisorDetailsPreview = document.getElementById('supervisorDetailsPreview');
        supervisorDetailsPreview.classList.remove('enlarged');
