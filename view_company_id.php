...
try {
    $stmt = $conn->prepare("
        ...
        WHERE 
            student_id = ? AND document_type = 'company_id'
        ORDER BY
            upload_date DESC
        LIMIT 1
    ");
    ...
    error_log("Company ID fetch error: " . $e->getMessage());
    $companyID = null;
}
?>
        ...
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            ...
            <div class="content-holder">
                <div class="file-overview-holder">
                    <fieldset>
                        <legend>Company ID Document</legend>
                        <div class="overview-cont">
                            <?php if ($companyID): ?>
                                <div class="moa-preview" id="companyIDPreview">
                                    <?php
                                    $file_ext = strtolower(pathinfo($companyID['uploaded_path'], PATHINFO_EXTENSION));
                                    if ($file_ext === 'pdf'): ?>
                                        <iframe src="<?php echo $companyID['uploaded_path'] . '?t=' . time(); ?>" frameborder="0"></iframe>
                                    <?php elseif (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                        <img src="<?php echo $companyID['uploaded_path']; ?>" alt="MOA Image">
                                    <?php else: ?>
                ...
                <div class="info-function-cont">
                        ...
                        <div class="moa-overview">
                            <?php if ($companyID): ?>
                                <div class="data-row">
                                    ...
                                    <div class="student-data">: <?php echo htmlspecialchars($companyID['document_name']); ?></div>
                                </div>
                                <div class="data-row">
                                    <div class="label"><p><b>Status</b></p></div>
                                    <div class="student-data">: <?php echo htmlspecialchars(ucfirst($companyID['status'])); ?></div>
                                </div>
                            <?php else: ?>
                                <div class="data-row">
                    ...
                    <div class="button-function">
                        <button class="btn-style" onclick="viewMoa()" <?php echo $companyID ? '' : 'disabled'; ?>>
                            <i class="fa-regular fa-eye"></i> View
                        </button>
                        <button class="btn-style" onclick="updateStatus(<?php echo $companyID['document_id'] ?? 'null'; ?>, 'accepted')" <?php echo $companyID && $companyID['status'] !== 'accepted' ? '' : 'disabled'; ?>>
                            <i class="fa-solid fa-circle-check"></i> Approve
                        </button>
                        <button class="btn-style" onclick="toggleReject()" <?php echo $companyID && $companyID['status'] !== 'denied' ? '' : 'disabled'; ?>>
...
function viewMoa() {
    <?php if ($companyID): ?>
        ...
        const companyIDPreview = document.getElementById('companyIDPreview');
        const overlay = document.getElementById('overlay');

        if (fileExt === 'pdf') {
            companyIDPreview.classList.toggle('enlarged');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    remarksBtn.addEventListener('click', function() {
        const documentId = <?php echo $companyID['document_id'] ?? 'null'; ?>;
    ...
    overlay.addEventListener('click', function() {
        const companyIDPreview = document.getElementById('companyIDPreview');
        companyIDPreview.classList.remove('enlarged');
