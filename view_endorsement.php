...
try {
    $stmt = $conn->prepare("
        ...
        WHERE 
            student_id = ? AND document_type = 'endorsement'
        ORDER BY
            upload_date DESC
        LIMIT 1
    ");
    ...
    error_log("endorsement Agreement fetch error: " . $e->getMessage());
    $endorsement = null;
}
?>
...
<head>
    ...
    <title>OJT Web Portal: endorsement Review</title>
        ...
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            ...
            <div class="content-holder">
                <div class="file-overview-holder">
                    <fieldset>
                        <legend>Endorsement document </legend>
                        <div class="overview-cont">
                            <?php if ($endorsement): ?>
                                <div class="moa-preview" id="endorsementPreview">
                                    <?php
                                    $file_ext = strtolower(pathinfo($endorsement['uploaded_path'], PATHINFO_EXTENSION));
                                    if ($file_ext === 'pdf'): ?>
                                        <iframe src="<?php echo $endorsement['uploaded_path'] . '?t=' . time(); ?>" frameborder="0"></iframe>
                                    <?php elseif (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                        <img src="<?php echo $endorsement['uploaded_path']; ?>" alt="MOA Image">
                                    <?php else: ?>
                                        <p class="no-file-message">Preview not available for this file type. Use the "View" button to open it.</p>
                                    <?php endif; ?>
                                </div>
                            <?php else: ?>
                                <p class="no-file-message">No endorsement document has been submitted.</p>
                            <?php endif; ?>
                        </div>
                    </fieldset>
                </div>
                <div class="info-function-cont">
                        ...
                        <div class="moa-overview">
                            <?php if ($endorsement): ?>
                                <div class="data-row">
                                    ...
                                    <div class="student-data">: <?php echo htmlspecialchars($endorsement['document_name']); ?></div>
                                </div>

                                <div class="data-row">
                                    <div class="label"><p><b>Status</b></p></div>
                                    <div class="student-data">: <?php echo htmlspecialchars(ucfirst($endorsement['status'])); ?></div>
                                </div>
                            <?php else: ?>
                                <div class="data-row">
                    ...
                    <div class="button-function">
                        <button class="btn-style" onclick="viewMoa()" <?php echo $endorsement ? '' : 'disabled'; ?>>
                            <i class="fa-regular fa-eye"></i> View
                        </button>
                        <button class="btn-style" onclick="updateStatus(<?php echo $endorsement['document_id'] ?? 'null'; ?>, 'accepted')" <?php echo $endorsement && $endorsement['status'] !== 'accepted' ? '' : 'disabled'; ?>>
                            <i class="fa-solid fa-circle-check"></i> Approve
                        </button>
                        <button class="btn-style" onclick="toggleReject()" <?php echo $endorsement && $endorsement['status'] !== 'denied' ? '' : 'disabled'; ?>>
...
<script>
    ...
    function viewMoa() {
        <?php if ($endorsement): ?>
            ...
            const endorsementPreview = document.getElementById("endorsementPreview");
            const overlay = document.getElementById("overlay");
        
        if (fileExt === 'pdf') {
            endorsementPreview.classList.toggle("enlarged");
            overlay.classList.toggle("active");
        } else {
            swal("Notice", "Preview not available for this file type", "info");
        }
        <?php else: ?>
            swal("Notice", "No endorsement document available for preview", "info");
        <?php endif; ?>
    }

    function updateStatus(documentId, status, remarks = null) {
        if (documentId === null) {
            swal("Error", "No endorsement document available to update.", "error");
            return;
        }
        swal({
            title: `Are you sure?`,
            text: `You are about to mark this endorsement as ${status}.`,
            type: "warning",
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        remarksBtn.addEventListener('click', function() {
            const documentId = <?php echo $endorsement['document_id'] ?? 'null'; ?>;
        ...
        overlay.addEventListener('click', function () {
            const endorsementPreview = document.getElementById("endorsementPreview");
            endorsementPreview.classList.remove("enlarged");
