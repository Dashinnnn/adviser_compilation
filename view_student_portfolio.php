...
$student_id = $_GET['id'] ?? '';
if (empty($student_id)) {
    header('Location: list_of_student_portfolio.php');
    exit;
}
...
try {
    $stmt = $conn->prepare("
        ...
        WHERE
            student_id = ? AND document_type = 'portfolio'
        ORDER BY
            upload_date DESC
        LIMIT 1
    ");
...
<head>
    ...
    <title>OJT Web Portal: Portfolio Review</title>
        ...
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            ...
            <div class="content-holder">
                <div class="file-overview-holder">
                    <fieldset>
                        <legend>Student Portfolio Document</legend>
                        <div class="overview-cont">
                            <?php if ($portfolio): ?>
                                <div class="moa-preview" id="portfolioPreview">
                                    <?php
                                    $file_ext = strtolower(pathinfo($portfolio['uploaded_path'], PATHINFO_EXTENSION));
                                    if ($file_ext === 'pdf'): ?>
                                        <iframe src="<?php echo $portfolio['uploaded_path'] . '?t=' . time(); ?>" frameborder="0"></iframe>
                                    <?php elseif (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                        <img src="<?php echo $portfolio['uploaded_path']; ?>" alt="Portfolio Image">
                                    <?php else: ?>
                                        <p class="no-file-message">Preview not available for this file type. Use the "View" button to open it.</p>
                                    <?php endif; ?>
                                </div>
                            <?php else: ?>
                                <p class="no-file-message">No Portfolio document has been submitted.</p>
                            <?php endif; ?>
                        </div>
                    </fieldset>
                </div>
                <div class="info-function-cont">
                    <div class="info-section">
                        ...
                        <div class="moa-overview">
                            <?php if ($portfolio): ?>
                                <div class="data-row">
                                    ...
                                    <div class="student-data">: <?php echo htmlspecialchars($portfolio['document_name']); ?></div>
                                </div>
                                <div class="data-row">
                                    <div class="label"><p><b>Status</b></p></div>
                                    <div class="student-data">: <?php echo htmlspecialchars($portfolio['status']); ?></div>
                                </div>
                            <?php else: ?>
                                <div class="data-row">
                                    <div class="label"><p><b>File Name</b></p></div>
                        ...
                        <div class="button-function">
                            <button class="btn-style" onclick="viewMoa()" <?php echo $portfolio ? '' : 'disabled'; ?>>
                                <i class="fa-regular fa-eye"></i> View
                            </button>
                            <button class="btn-style" onclick="updateStatus(<?php echo $portfolio['document_id'] ?? 'null'; ?>, 'accepted')" <?php echo $portfolio && $portfolio['status'] !== 'accepted' ? '' : 'disabled'; ?>>
                                <i class="fa-solid fa-circle-check"></i> Approve
                            </button>
                            <button class="btn-style" onclick="toggleReject()" <?php echo $portfolio && $portfolio['status'] !== 'denied' ? '' : 'disabled'; ?>>
    ...
    <script>
        ...
        function viewMoa() {
            <?php if ($portfolio): ?>
                ...
                if (fileExt === 'pdf') {
                    portfolioPreview.classList.toggle('enlarged');
            ...
            <?php else: ?>
                showCustomAlert({
                    title: 'Notice',
                    html: 'No portfolio document available to view.',
                    icon: 'info',
                    confirmText: 'OK'
                });
            <?php endif; ?>
        ...
        function updateStatus(documentId, status, remarks = null) {
            if (documentId === null) {
                showCustomAlert({
                    title: 'Error',
                    html: 'No portfolio document available to update.',
                    icon: 'error',
                    confirmText: 'OK'
                });
                return;
            ...
            showCustomAlert({
                title: 'Confirm',
                html: `You are about to mark this portfolio as <strong>${status}</strong>.`,
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            remarksBtn.addEventListener('click', function() {
                const documentId = <?php echo $portfolio['document_id'] ?? 'null'; ?>;
            ...
            overlay.addEventListener('click', function() {
                const portfolioPreview = document.getElementById('portfolioPreview');
                portfolioPreview.classList.remove('enlarged');
