...
try {
    $stmt = $conn->prepare("
        ...
        WHERE 
            student_id = ? AND document_type = 'coc'
        ORDER BY
            upload_date DESC
        LIMIT 1
    ");
...
<head>
    ...
    <title>OJT Web Portal: Certificate of Completion Review</title>
        ...
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            ...
            <div class="content-holder">
                <div class="file-overview-holder">
                    <fieldset>
                        <legend>Certificate of Completion Document</legend>
                        <div class="overview-cont">
                            <?php if ($coc): ?>
                                <div class="moa-preview" id="moaPreview">
                                    <?php
                                    $file_ext = strtolower(pathinfo($coc['uploaded_path'], PATHINFO_EXTENSION));
                                    if ($file_ext === 'pdf'): ?>
                                        <iframe src="<?php echo $coc['uploaded_path'] . '?t=' . time(); ?>" frameborder="0"></iframe>
                                    <?php elseif (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                        <img src="<?php echo $coc['uploaded_path']; ?>" alt="COC Image">
                                    <?php else: ?>
                                        <p class="no-file-message">Preview not available for this file type. Use the "View" button to open it.</p>
                                    <?php endif; ?>
                                </div>
                            <?php else: ?>
                                <p class="no-file-message">No Certificate of Completion document has been submitted.</p>
                            <?php endif; ?>
                        </div>
                    </fieldset>
                </div>
                <div class="info-function-cont">
                        ...
                        <div class="moa-overview">
                            <?php if ($coc): ?>
                                <div class="data-row">
                                    ...
                                    <div class="student-data">: <?php echo htmlspecialchars($coc['document_name']); ?></div>
                                </div>
                                <div class="data-row">
                                    <div class="label"><p><b>Status</b></p></div>
                                    <div class="student-data">: <?php echo htmlspecialchars(ucfirst($coc['status'])); ?></div>
                                </div>
                            <?php else: ?>
                                <div class="data-row">
                    ...
                    <div class="button-function">
                        <button class="btn-style" onclick="viewMoa()" <?php echo $coc ? '' : 'disabled'; ?>>
                            <i class="fa-regular fa-eye"></i> View
                        </button>
                        <button class="btn-style" onclick="updateStatus(<?php echo $coc['document_id'] ?? 'null'; ?>, 'accepted')" <?php echo $coc && $coc['status'] !== 'accepted' ? '' : 'disabled'; ?>>
                            <i class="fa-solid fa-circle-check"></i> Approve
                        </button>
                        <button class="btn-style" onclick="toggleReject()" <?php echo $coc && $coc['status'] !== 'denied' ? '' : 'disabled'; ?>>
    ...
    <script>
        ...
        function viewMoa() {
            ...
            <?php else: ?>
                showCustomAlert({
                    title: 'Notice',
                    html: 'No Certificate of Completion document available to view.',
                    icon: 'info',
                    confirmText: 'OK'
                });
            <?php endif; ?>
        ...
        function updateStatus(documentId, status, remarks = null) {
            if (documentId === null) {
                showCustomAlert({
                    title: 'Error',
                    html: 'No Certificate of Completion document available to update.',
                    icon: 'error',
                    confirmText: 'OK'
                });
                return;
            ...
            showCustomAlert({
                title: 'Confirm',
                html: `You are about to mark this Certificate of Completion as <strong>${status}</strong>.`,
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            remarksBtn.addEventListener('click', function() {
                const documentId = <?php echo $coc['document_id'] ?? 'null'; ?>;
