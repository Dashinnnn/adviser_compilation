...
try {
    $stmt = $conn->prepare("
        ...
        WHERE 
            student_id = ? AND document_type = 'coe'
        ORDER BY
            upload_date DESC
        LIMIT 1
    ");
    $stmt->execute([$student_id]);
    $coe = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    error_log("COE fetch error: " . $e->getMessage());
    $coe = null;
}
?>
        ...
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            ...
            <div class="content-holder">
                <div class="file-overview-holder">
                    <fieldset>
                        <legend>Certificate of Employment Document</legend>
                        <div class="overview-cont">
                            <?php if ($coe): ?>
                                <div class="moa-preview" id="coePreview">
                                    <?php
                                    $file_ext = strtolower(pathinfo($coe['uploaded_path'], PATHINFO_EXTENSION));
                                    if ($file_ext === 'pdf'): ?>
                                        <iframe src="<?php echo $coe['uploaded_path'] . '?t=' . time(); ?>" frameborder="0"></iframe>
                                    <?php elseif (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                        <img src="<?php echo $coe['uploaded_path']; ?>" alt="MOA Image">
                                    <?php else: ?>
                ...
                <div class="info-function-cont">
                        ...
                        <div class="moa-overview">
                            <?php if ($coe): ?>
                                <div class="data-row">
                                    ...
                                    <div class="student-data">: <?php echo htmlspecialchars($coe['document_name']); ?></div>
                                </div>
                                <div class="data-row">
                                    <div class="label"><p><b>Status</b></p></div>
                                    <div class="student-data">: <?php echo htmlspecialchars(ucfirst($coe['status'])); ?></div>
                                </div>
                            <?php else: ?>
                                <div class="data-row">
                    ...
                    <div class="button-function">
                        <button class="btn-style" onclick="viewMoa()" <?php echo $coe ? '' : 'disabled'; ?>>
                            <i class="fa-regular fa-eye"></i> View
                        </button>
                        <button class="btn-style" onclick="updateStatus(<?php echo $coe['document_id'] ?? 'null'; ?>, 'accepted')" <?php echo $coe && $coe['status'] !== 'accepted' ? '' : 'disabled'; ?>>
                            <i class="fa-solid fa-circle-check"></i> Approve
                        </button>
                        <button class="btn-style" onclick="toggleReject()" <?php echo $coe && $coe['status'] !== 'denied' ? '' : 'disabled'; ?>>
...
function viewMoa() {
    <?php if ($coe): ?>
        ...
        const coePreview = document.getElementById('coePreview');
        const overlay = document.getElementById('overlay');

        if (fileExt === 'pdf') {
            coePreview.classList.toggle('enlarged');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    remarksBtn.addEventListener('click', function() {
        const documentId = <?php echo $coe['document_id'] ?? 'null'; ?>;
    ...
    overlay.addEventListener('click', function() {
        const coePreview = document.getElementById('coePreview');
        coePreview.classList.remove('enlarged');
